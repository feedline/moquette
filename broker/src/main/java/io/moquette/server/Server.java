...
public class Server {
    ...
    public void externalPublish(MqttPublishMessage msg, final String clientId) {
        final int messageID = msg.variableHeader().packetId();
        if (!m_initialized) {
            LOG.error("Moquette is not started, internal message cannot be published. CId={}, messageId={}", clientId, messageID);
            throw new IllegalStateException("Can't publish on a server is not yet started");
        ...
        if(LOG.isDebugEnabled())
            LOG.debug("External Publishing message. CId={}, messageId={}", clientId, messageID);

        // send local
        m_processor.internalPublish(msg, clientId);

        // broadcast
        ITopic<HazelcastMsg> topic = getHazelcastInstance().getTopic("moquette");
        MoquetteMessage moquetteMessage = new MoquetteMessage(msg.fixedHeader(), msg.variableHeader(), msg.content());
        HazelcastMsg hazelcastMsg = new HazelcastMsg(new InterceptPublishMessage(moquetteMessage, clientId,null));
        topic.publish(hazelcastMsg);
        msg.content().release(); // 防止内存泄漏
    }
